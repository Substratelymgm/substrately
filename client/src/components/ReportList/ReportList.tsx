...
const ReportList: React.FC<ReportListProps> = ({ reportItems }) => {
  ...
  const handleDotsClick = (event: React.MouseEvent, item: ReportItem) => {
    event.stopPropagation();
    // Add your custom functionality here
    console.log('Dots clicked for:', item);
    // For example, you can open another modal or display options specific to the item
  };
      ...
      <Modal isOpen={isReportItemsModalOpen} onClose={closeReportItemsModal}>
        <div className='bg-white w-full max-w-[556px] rounded-lg min-h-[290px]'>
          ...
          <div className="w-full flex items-center justify-center mb-[2rem] mt-[1rem]">
            <button className='p-2 bg-[#3B8F85] w-full max-w-[223px] mx-auto text-white p-2 rounded-md' onClick={closeReportItemsModal}>Close</button>
          </div>
        </div>
      </Modal>
        ...
        {reportItems.map((item: ReportItem, index) => (
          <div onClick={() => handleViewReport(item)} key={index} className="p-4 hover:scale-[105%] duration-300 border bg-white rounded-md gap-[1rem] flex flex-col items-center">
            <div className="flex w-full justify-between items-center">
              ...
              <div className="w-[30px] h-[30px] rounded-full flex items-center justify-center hover:bg-gray-100 cursor-pointer duration-300" onClick={(e) => handleDotsClick(e, item)}>
